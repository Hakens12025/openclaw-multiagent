...
> 活跃刷新: 2026-08-09 (~v181-stable)，operator 手已通（operatorExecutable surface + executor 落地 + structure-snapshot 回滚 + forceVerify）、designer-only 边界确立；AgentGroup/ProfileLifecycle/HarnessModule 均已落地。前序：v115 四关节自治回路物理闭合。
...
| [平台/Agent 解耦](concepts/platform-agent-decoupling.md) | 判据:没读过文档的新 agent 进来能不能干活;平台提供服务而非要求 agent 内部逻辑 |

## 系统架构

| 页面 | 摘要 | 状态 |
|------|------|------|
| [七层系统分层](concepts/system-layering.md) | **职责**模型 L0内核→L1通讯→L2控制面→L3执行→L4评估→L5治理→L6投影。⚠️ 定址用的是 `docs/system-map.md` 的 11 层 L0-L10，两套 `L{n}` 别混 | 概念稳定 |
...
| [Graph Edge](concepts/graph-edge.md) | **固定管线**的定义;动态协作已不查图边,`metadata.pipeline` 分开两种读法 | 演化中 |
| [合约 (Contract)](concepts/contract.md) | 任务载体，携带 assignee/replyTo，DRAFT 已消除 | 核心稳定 |
| [记录面 (Record Plane)](concepts/record-plane.md) | 记账收口 SQLite 单库:run_event/trace_event 两账,(sessionKey,seq) 唯一索引拒双写者;文件账/哈希链已于 v232 退役 | **终态(2026-08-24)** |
| [Loop](concepts/loop.md) | **已退役 2026-08-18**：LoopSpec/LoopSession/start_loop/graph.loop.* 全删；环由 detectCycles 识别，时间维回落到传送带逐跳推进 | 已退役（历史页） |
| [投递 (Delivery)](concepts/delivery.md) | 统一结果回送：terminal + system_action return variants | 稳定 |
| [Agent 工作空间](concepts/agent-workspace.md) | 物理隔离的空间模型;三类空间与权限;**交接是它的推论** | 稳定/有未决张力 |
| [产物交接](concepts/artifact-handoff.md) | 产物按 producer 整箱流转、不进提示词；七条不变式 + 五个关键设计选择;取件条带清单、采集不再要令牌 | 固定管线稳定 / 动态腿+提交面演化中 |
| [Session 管理](concepts/session-management.md) | 合约独立 session，运行时主链已支持 deterministic session key | 部分实现 |
| [CLI System](concepts/cli-system.md) | 正式可操作表面层：五族真值边界裁定（inspect 观测读唯一入口，计数见 `lib/cli-system/cli-surface-catalog.js`（2026-08-09 抽样 inspect 35 / total 40）；apply=admin-operations；HTTP 投影 /watchdog/inspect） | 全族收口 |
...
| [AgentGroup](concepts/agent-group.md) | 图原语：空间封装（对照的「时间原语」Loop 已于 2026-08-18 退役） | 已落地（agent_groups surface + v119 宏展开） |
...
| [Operator](concepts/operator.md) | designer-only meta-agent：设计 structure + agent 内容，不替用户跑具体任务；「去伪」=拆 if-else 还原真 agent | 手已通（executor 落地+回滚+forceVerify） |
| [Workspace 引导](concepts/workspace-guidance.md) | agent 文档层级：执行层四角色只有 IDENTITY+COLLABORATION-FALLBACK+HEARTBEAT，别的 5 份会被主动删除 | 已实现 |

## 执行与治理

| 页面 | 摘要 | 状态 |
|------|------|------|
| [Harness](concepts/harness.md) | 执行塑形层 | **已退役 v226（2026-08-23）**，见备忘录149/150；「标准化」思想保留 |
...
| [测试系统](concepts/test-system.md) | test-runner.js 唯一入口；预设清单以 `--list` live 输出为准(默认 health 零 LLM 体检)，CheckResult+E-* 错误码注册表，failures-first 报告 | 功能可用 |
...
| [Agent 即分类器](decisions/agent-as-classifier.md) | 删除 regex 预分类，LLM 自然判断（`[DISPATCH]` 标记形态已作废，结论不变） | 03-18 |
| [Graph 作为运行时真值](decisions/graph-as-runtime-truth.md) | 运行时强制执行，不只是 UI 装饰（协作授权部分已被下一条取代） | 03-19 |
| [动态协作不查图边](decisions/dynamic-collaboration-leaves-graph.md) | 图=固定管线定义;动态协作自己指定目标,约束回到角色策略 | 08-08 |
| [平台服务 FC 族](decisions/platform-service-fc-family.md) | submit_output 先行(扛 failed/awaiting_input/hold);submit_plan+report_progress 成对缓建 | 08-09 |
| [God Role 消除](decisions/god-role-elimination.md) | 只保留 bridge+worker，其余为 policy+skill 组合 | 03-31 |
| [历史编排引擎收口](decisions/pipeline-dissolution.md) | 旧编排职责回收到 loop-session 与 dispatch graph policy（loop-session 本身已于 2026-08-18 退役） | 04-03 |
| [统一控制面](decisions/unified-control-plane.md) | 六路分发收敛为 graph+ingress+conveyor+lifecycle（第五路 loop engine 已于 2026-08-18 退役） | 03-26 |
| [Agent-系统交互最小化](decisions/agent-system-interaction-minimization.md) | [ACTION] 标记替代 JSON 文件协议（"标记是唯一通道"那半已被 v179 FC 工具主路取代，原则本体仍成立） | 04-07 |
...
| [wake 提示词数据驱动](decisions/wake-prompt-role-driven.md) | wake 提示词按 role-spec 派生 per-role 个性（`soul-template-builder.js` 已删，结论不变） | 05-31 |
| [环 ≠ 受控 loop](decisions/cycle-vs-registered-loop.md) | **SUPERSEDED 2026-08-18**：上半句（环只是拓扑）升为唯一真值，下半句（注册成 LoopSpec）随回路退役作废 | 05-31 |
| [role/SOUL/wake 解耦+英文化](decisions/role-soul-wake-decoupling.md) | SOUL=纯用户；role→IDENTITY(④托管)；wake 叠加仅派工；三层提示词改英文 | 06-10 |
| [test-runner 大修](decisions/test-runner-overhaul-2026-06.md) | CheckResult+E-码注册表+failures-first 报告;19→8 预设(默认 health,后增至 14,回路退役后 13);verify 门→dispatch(2026-08 预设改名后=single) | 06-10 |
