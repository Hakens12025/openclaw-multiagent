...
`agents.defaults.model.fallbacks = [providerRef, ...]`（有序，**核心原生字段**——`AgentModelSchema` 本就声明 `{primary, fallbacks}` strict，单数 `fallback` 会被 schema 拒导致网关拒启）声明 meta-agent brain（operator / viz-master）的兜底 provider。`resolveBrainModelChain(config)`（`lib/llm/brain-model-resolver.js`）解析出 `[primary, ...ready fallbacks]`——非 `openai-completions` / 无 baseUrl / 无 key 的 ref 跳过，fullRef 去重；`callPlannerWithModelFallback`（`lib/operator/operator-brain.js`，operator 与 viz-master 共享）walk 这条链。空 fallback = 旧的单解析行为（向后兼容，`resolveOperatorBrainModel` 字节不变）。
...
**关键边界**：只在 **provider 类错误**（限额/socket/超时/http）兜底；`PLANNER_JSON_PARSE_FAILED`（内容错误，模型答了但不可解析）**不跨 provider**——否则把同一份坏推理换 provider 重跑、错标失败类（尊重 `lib/llm/llm-planner.js` 既有区分）。
...
**已生效（v161→v162）。** 全局 `agents.defaults.model.fallbacks` live 已设 `["glm-bigmodel/glm-5.1"]`，网关已重启加载（2026-06-09 实证：零校验错误 + live 链解析正确）。per-agent 链待 brain-resolver 泛化 per-agent 时加；dashboard 监听 `provider_fallback` 事件的前端白名单待加。
