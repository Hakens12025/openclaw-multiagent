...
```bash
node --test /Users/hakens/.openclaw/extensions/watchdog/tests/dashboard-route-modules.test.js
curl -I 'http://localhost:18789/watchdog/work-items-view?token=YOUR_GATEWAY_TOKEN'
```
...
```bash
curl -sS 'http://localhost:18789/watchdog/work-items?token=YOUR_GATEWAY_TOKEN' | head
```
...
```text
http://localhost:18789/watchdog/work-items-view?token=YOUR_GATEWAY_TOKEN
```
