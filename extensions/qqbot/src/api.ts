...
// 代理支持：读取 HTTPS_PROXY 或 ALL_PROXY 环境变量
const _proxyUrl = process.env.HTTPS_PROXY || process.env.ALL_PROXY || process.env.HTTP_PROXY;
...
async function doFetchToken(appId: string, clientSecret: string): Promise<string> {
  ...
  try {
    response = await apiFetch(TOKEN_URL, {
      method: "POST",
