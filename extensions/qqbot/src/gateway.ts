...
请根据图片内容进行回复。`);
        ...
        // 添加当前用户信息，帮助 AI 识别对话者
        const currentUserInfo = `【当前用户信息】
用户ID: ${event.senderId}
用户名称: ${event.senderName ?? "未知"}`;
              ...
              deliver: async (payload: { text?: string; mediaUrls?: string[]; mediaUrl?: string }, info: { kind: string }) => {
                ...
                if (dispatchMatch) {
                  ...
                  try {
                    ...
                    if (!dispatched) {
                      receiptText = "\u26A0\uFE0F 任务提交失败，请稍后再试";
                    }
                  } catch (err) {
                    log?.error(`[qqbot:${account.accountId}] DISPATCH error: ${err}`);
                    receiptText = "\u26A0\uFE0F 任务提交异常，请稍后再试";
                  }
                  try {
                    await sendWithTokenRetry(async (token) => {
                      if (event.type === "c2c") {
                        await sendC2CMessage(token, event.senderId, receiptText, event.messageId);
                      } else if (event.type === "group" && event.groupOpenid) {
                        await sendGroupMessage(token, event.groupOpenid, receiptText, event.messageId);
                      } else if (event.channelId) {
                        await sendChannelMessage(token, event.channelId, receiptText, event.messageId);
                      }
                    });
                  } catch (err) {
                    log?.error(`[qqbot:${account.accountId}] Failed to send DISPATCH receipt: ${err}`);
                  }
                  pluginRuntime.channel.activity.record({
                    channel: "qqbot",
                    accountId: account.accountId,
