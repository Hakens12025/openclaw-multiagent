// lib/operator/wiki-rag-search.js — operator-facing wiki-RAG query entry.
//
...
export async function hybridSearchOverIndex(queryText, index, { topK = 5, asOf = null, embedModel = null } = {}) {
  ...
  const lexical = await searchWikiRagLexical(q, { topK: wide, index: scopedIndex }); // 词法不需 ollama,总能算
  ...
  const fused = rrfFuse([vector, lexical], { topK: wide }); // wide 供分歧;results 仍切 topK(召回零回归)
  const out = { ok: true, degraded, results: fused.slice(0, Math.max(1, Number(topK) || 5)) };
