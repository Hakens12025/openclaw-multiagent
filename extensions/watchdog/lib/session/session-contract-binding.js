...
import { getContractPath, mutateContractById, updateContractStatus } from "../contract/contracts.js";
...
import { ensureRuntimeDirectEnvelopeInbox } from "../routing/runtime-direct-envelope-queue.js";
...
import { wireClaimed } from "../archive/run-event-wiring.js";
...
export async function bindPendingWorkerContract({
  ...
  applyContractLineageToTracking(trackingState, contract);
  notifyTrackingContractClaim(sessionKey, trackingState.contract.id);
  await mutateContractById(contract.id, logger, (shared) => {
    shared.status = CONTRACT_STATUS.RUNNING;
  }, {
    touchUpdatedAt: true,
    logMessage: (shared) => `[watchdog] contract ${shared.id} → ${CONTRACT_STATUS.RUNNING}`,
  });
  await claimDispatchTargetContract({ contractId: contract.id, agentId, logger });
  notifyDispatchTargetClaim(agentId, contract);
  // 事件接线(批② §八):tracker bind 完成 = claimed 时刻。fire-and-forget。
  void wireClaimed({ contract, agentId, sessionKey, logger });
...
export async function bindInboxContractEnvelope({
  ...
  try {
    ...
    applyContractLineageToTracking(trackingState, binding.contract);
    if (
      !isDirectRequest
    ...
    // 事件接线(批② §八):信封/inbox 副本绑定 = claimed 时刻(谱系取重解析后的正本)。
    void wireClaimed({
