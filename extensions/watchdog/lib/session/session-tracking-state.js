...
} from "../routing/route-metadata.js";
...
import { buildInputIoObservation, mergeIoObservation } from "../stage/io-observation.js";
...
export function toTrackingContract(contract, path) {
  ...
  const trackingContract = {
    ...
    // 批① 谱系:白名单式抄写 —— 漏抄=tracker 视角丢字段(contract.output 镜像 bug 同构)。
    lineage: contract.lineage || null,
    stagePlan,
    stageRuntime,
    ...
    expectations: contract.expectations || null,
    ...
    // 回路运行时退役(2026-08-18):此处曾抄写 contract.pipelineStage(路由段:
    // stage/round/pipelineId/loopId/loopSessionId)。阶段真值单源已是
    // stagePlan + stageRuntime,路由段整对象随回路退役,不再进 tracker 视角。
...
export function createTrackingState({ sessionKey, agentId, parentSession, runId = null, executionPolicy = null }) {
  return {
    ...
    // 批① 谱系:合约绑定时由 applyContractLineageToTracking 回填(覆盖上面的随机
    // runId);无谱系约保持随机=现状,threadId 保持 null。
    threadId: null,
    executionPolicy,
    startMs: Date.now(),
    toolCalls: [],
