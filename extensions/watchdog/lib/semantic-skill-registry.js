...
const SEMANTIC_SKILL_SPECS = Object.freeze({
  ...
  "system-action": Object.freeze({
    id: "system-action",
    name: "System Action",
    summary: "平台协作入口。协作动作写在 [ACTION] 标记里。",
    ...
    priority: 8,
    tags: Object.freeze(["system-action", "assign_task", "request_review", "wake_agent", "loop", "runtime-graph"]),
    pluginRefs: Object.freeze(["watchdog"]),
    toolRefs: Object.freeze(["assign_task", "request_review", "wake_agent", "start_loop", "advance_loop"]),
    guideLine: "协作动作统一写在输出中的 [ACTION] 标记里。",
    operatorUse: "平台调度技能：agent 需要委派、审查、唤醒或启动 loop 时应通过 system_action 交给 runtime；loop 推进由 runtime graph 与 outbox 接管。",
  }),
