...
export async function resolveTerminalUserFacingResultContent(source) {
  // 读序:①树封包 seal.primary(合约轮正本) ②participants delivery-{cid}.md
  // 快照 ③既有路径解析(terminalOutcome/observation 路径与 contract.output
  // 镜像遗产、直写链路)。前两级凭合约 id 经 contract-index 找家,无家自然落空。
  const sourceContractId = (source && typeof source !== "string")
    ? source?.contract?.id || source?.contractId || null
    : null;
  if (sourceContractId) {
    // assignee = 最后一跳受托人:多跳同 cid 多封条时按它锁定终局封条。
    const sourceAssignee = (source && typeof source !== "string")
      ? source?.contract?.assignee || source?.assignee || null
      : null;
    const sealed = findSealedOutbox(sourceContractId, { agentId: sourceAssignee });
    const treeCandidates = [
      sealed?.primaryPath,
      findDeliverySnapshotPath(sourceContractId, { agentId: sourceAssignee }),
    ].filter(Boolean);
    for (const candidate of treeCandidates) {
      try {
        const artifactText = await readFile(candidate, "utf8");
...
export function buildRuntimeResultDeliveryTask({
  ...
  const statusText = (terminalStatus || "unknown");
  ...
  const statusLine = failureInstruction;
