// lib/routing/delivery/delivery-result-extract.js — user-facing text extraction from runtime artifacts
//
...
// 裸机器码判据：整串只由标识符字符组成（tool_write_failed / missing_output / E-123）。
// 这类值是给 grep 用的键，对收件人是噪音，与协议行话同等降级。中文摘要含 CJK 与全角
// 标点，落不进这个字符集，因此不会被误伤。
const MACHINE_CODE_RE = /^[\w.:+-]+$/u;

// 失败说明的长度上限：summary 可能来自 agent 自撰的 stage 摘要，长度无保证。
// 截断既保住「简洁」，也保证末尾的定位行落在调用方 1500 字切片之内。
const FAILURE_DETAIL_MAX_CHARS = 600;

// 对外失败文案只接受「给人读的句子」：既非协议行话，也非裸机器码。
function resolveHumanFailureDetail(text) {
  const normalized = String(text || "").trim();
  if (!normalized || isInternalDeliveryReason(normalized) || MACHINE_CODE_RE.test(normalized)) {
    return "";
  }
  return normalized.length > FAILURE_DETAIL_MAX_CHARS
    ? `${normalized.slice(0, FAILURE_DETAIL_MAX_CHARS)}…`
    : normalized;
}

// 失败定位线索：语义阶段 id（跨 agent 稳定）。回路运行时退役（2026-08-18）后
// contract.pipelineStage 路由段已不存在，取值链收口到 stageRuntime 单源——
// 用户可见的「📍 环节」文案由这一条供给。
export function resolveContractStageLabel(contract) {
  return String(contract?.stageRuntime?.currentStageId || "").trim();
}

// FAILED 对外文案的唯一出口，QQ 目标腿与内部 replyTo 腿共用。
// summary 是唯一为人书写的字段（handoff gate 拦截说明、hard-stop 说明、stage 摘要），
// 因此排在 reason 之前；clarification 刻意不参与——它的语义是「请补充什么输入」，
// 属已删除的 AWAITING_INPUT 时代;正则里的词保留用于清洗旧产物文本。
export function buildUserFacingFailureText({ summary = null, reason = null, stage = null } = {}) {
  const detail = resolveHumanFailureDetail(summary)
    || resolveHumanFailureDetail(reason)
    || "任务未完成。";
