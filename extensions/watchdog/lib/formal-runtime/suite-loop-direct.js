// Direct loop-start surface regression cases for the formal runtime runner.
...
import { loadGraphLoopRegistry, saveGraphLoopRegistry } from "../loop/graph-loop-registry.js";
import { executeAdminSurfaceOperation } from "../admin/admin-surface-operations.js";
import { loadActiveLoopRuntime } from "../loop/loop-round-runtime.js";
...
function resolveLoopWakeEvidence(startResult, heartbeatCalls) {
  const wake = startResult?.wake && typeof startResult.wake === "object"
    ? startResult.wake
    : null;
  const heartbeat = Array.isArray(heartbeatCalls) && heartbeatCalls.length > 0
    ? heartbeatCalls[0]
    : null;
  const targetAgent = wake?.targetAgent || heartbeat?.agentId || null;
  return {
    ok: wake?.ok === true || Boolean(heartbeat),
    targetAgent,
    detail: wake || heartbeat || [],
  };
}
...
async function cleanDirectLoopArtifacts(entryAgentId) {
  ...
  if (entryAgentId) {
    ...
    await rm(join(getEntryAgentInbox(entryAgentId), ".runtime-direct-envelope-queue"), { recursive: true, force: true }).catch(() => {});
  }
  await rm(CONTRACTOR_CONTRACT_FILE, { force: true }).catch(() => {});
  await rm(join(CONTRACTOR_INBOX, ".runtime-direct-envelope-queue"), { recursive: true, force: true }).catch(() => {});
}
...
export async function runLoopDirectCase(testCase) {
  ...
  try {
    ...
    const wakeEvidence = resolveLoopWakeEvidence(startResult, heartbeatCalls);
    if (!wakeEvidence.ok || wakeEvidence.targetAgent !== entryAgentId) {
      results.push({
        id: "D4",
        name: "Unified wake targets entry agent",
        status: "FAIL",
        elapsed: elapsed(),
        detail: trimDetail({
          wake: startResult?.wake || null,
          heartbeatCalls,
        }),
      });
      return {
    ...
    results.push({
      ...
      status: "PASS",
      elapsed: elapsed(),
      detail: trimDetail(wakeEvidence.detail),
    });
...
export function generateLoopDirectReport(testResults, totalDuration) {
  ...
  for (const result of testResults) {
    ...
    if (result.loopRuntimeSnapshot) lines.push(`  LoopRuntime: ${trimDetail(result.loopRuntimeSnapshot)}`);
