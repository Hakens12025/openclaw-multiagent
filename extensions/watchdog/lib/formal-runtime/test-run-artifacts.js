// lib/formal-runtime/test-run-artifacts.js — 报告落盘（文件命名/目录不变：devtool-<presetId>-<ts>.txt/.json）
// .txt = generateFormalReport（failures-first 布局）；.json = buildFormalReportJson（机器可读镜像）。

import { mkdir, writeFile } from "node:fs/promises";
import { join } from "node:path";

import { buildFormalReportJson, generateFormalReport } from "./formal-report.js";
import { PORT } from "./infra-tokens.js";
