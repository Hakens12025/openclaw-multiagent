// T2 platform scenario cases for explicit loop control-plane start.
...
export const LOOP_PLATFORM_CASES = [
  {
    ...
    transportPath: ["graph.loop.compose", "runtime.loop.start", "conveyor.dispatch", "loop runtime", "lifecycle.commit"],
    expectedRuntimeTruth: ["loop truth registered", "runtime loop start accepted", "loop runtime truth activated", "entry stage owned by entry agent", "heartbeat targets entry agent"],
    coverage: ["loop_control_plane", "loop_runtime_truth", "entry_dispatch", "frontend_visibility"],
  },
];
...
const CHECKPOINT_META = {
  ...
  D2: {
    ...
    subsystem: "loop-runtime-truth",
    conclusion: "loop runtime 或 loop session 与显式 loop 启动真值不一致。",
    suggestedFix: "检查 runtime.loop.start、loop runtime 写入与 loop session 写入。",
  },
  D3: {
    id: "P3",
  ...
  D4: {
    ...
    subsystem: "loop-wake",
    conclusion: "显式 loop 启动后没有通过统一 wake 通道正确唤醒当前 entry agent。",
    suggestedFix: "检查 dispatch wake 结果、hooks/heartbeat 通道与 entry agent 目标解析。",
  },
};
...
export async function runLoopPlatformTest(testCase, _sse = null) {
  ...
  return {
    ...
    results: Array.isArray(directResult.results)
      ...
      : [],
    contractRuntime: {
      loopRuntime: directResult.loopRuntimeSnapshot || null,
