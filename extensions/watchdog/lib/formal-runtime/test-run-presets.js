// lib/formal-runtime/test-run-presets.js — preset 解析与 case 收集（CheckResult 体系）
//
// case 真值内联在各 suite 模块里（不再有独立 case catalog）：
//   single/pipeline → lib/formal-runtime/suite-link.js（DISPATCH/PIPELINE_LINK_CASES）
//   collab          → lib/formal-runtime/checks/system-action-chain.js（SYSTEM_ACTION_CASES，
//                     suite 驱动整组执行，不支持 per-case 子集）
//   health/operator/knowledge → 自寻靶，无 case 概念
// 因此 --case 只对 single/pipeline 的 case id 生效。

import { listFormalTestPresets } from "./formal-test-presets.js";
import { DISPATCH_LINK_CASES, PIPELINE_LINK_CASES } from "./suite-link.js";
...
// 旧预设名墓碑（2026-08 改名波）：命中旧名硬报错并指路新名，风格沿用 CLI 退役旗标
// （test-runner-cli-client.js 的 RETIRED_CLI_FLAGS）。providers 由 model 建设者吸收。
// （"harness"→"automation-eval" 墓碑已随 harness 全退役整体移除，v226 / 2026-08-23）
const RENAMED_PRESETS = Object.freeze({
  "dispatch": "single",
...
export function collectDispatchCases(caseIds) {
  return collectCases(caseIds, DISPATCH_LINK_CASES, "single");
}
...
function resolveAdHocCasePreset(caseId) {
  ...
  const candidates = [
    { suite: "single", transport: "isolated", collect: collectDispatchCases },
...
export function resolveRequestedFormalPreset({
  ...
  if (normalizedPresetId) {
    if (Object.hasOwn(RENAMED_PRESETS, normalizedPresetId)) {
      throw new Error(`renamed preset: "${normalizedPresetId}" is now "${RENAMED_PRESETS[normalizedPresetId]}"`);
    }
  ...
  if (normalizedCaseId) {
    ...
    if (!preset) {
      throw new Error(`unknown case: ${normalizedCaseId} (per-case runs support single/pipeline case ids only)`);
    }
    return preset;
  }
