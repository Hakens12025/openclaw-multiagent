// lib/knowledge/knowledge-base-registry.js — 用户/operator 注册的知识库持久层(镜像 automation-registry)。
//
...
//
// v185 补 scope/agentId:此前 scope 只在 addKnowledgeBaseSource **新建**分支里生效
// (`base = existing || {…}`,merged 又原样展开 base),已存在的库**没有任何路径能重新划分可见性**
// —— 只能删库重建,而重建 = 全量重嵌(memos 实测 285s)。实际代价:memos 建成 global 后
// 自动进了所有 includeGlobal 检索方(operator/viz-master grounding),与
// operator-knowledge.js「memos = RAW past designs that mislead,不作 grounding」的书面约束冲突,
// 却改不回来。scope 是可见性真值,必须可改;补在本函数(既有的"配置这个库"入口),不另开路径。
// 注:scope 只决定**自动可见性**(selectAgentKnowledgeBases);按 kbId 直接寻址的路径
// (searchKb / 评测 / kb-ask --kb)不受影响,降级不会让库变得不可达。
export async function configureKnowledgeBase(kbId, { temporal, metadataRules, conflict, scope, agentId } = {}) {
  const id = normalizeString(kbId);
  if (!id) throw new Error("missing knowledge base id");
  ...
  return withLock(KNOWLEDGE_BASES_LOCK, async () => {
    ...
    // scope 与 agentId 一起算:改 scope 而不同时给 agentId,旧的 agentId 会残留成矛盾状态
    // (scope=global 却挂着 agentId,或 scope=agent 沿用上一个 agent)。故 scope 一旦出现就
    // 重算两者,agentId 未给则归 null = 已注册但未绑定任何 agent(合法状态:
    // selectAgentKnowledgeBases 要求 agentId 全等于非空 agentId,null 永不匹配)。
    const rescope = scope !== undefined
      ? { scope: scope === "agent" ? "agent" : "global", agentId: scope === "agent" ? (normalizeString(agentId) || null) : null }
      : (agentId !== undefined && existing.scope === "agent" ? { agentId: normalizeString(agentId) || null } : {});
    const merged = {
      ...existing,
