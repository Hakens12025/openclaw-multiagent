// lib/knowledge/knowledge-base.js — 多知识库注册表 + ingestion(Phase 2)
//
...
// indexPath 覆盖(默认 null = 正式落点):A/B 实验把影子索引建到别处,正式索引保持原样(rag-todo T3/T6 前置)。
// 缺省时两分支与覆盖前逐字节等价:wiki 分支零透传该键(buildWikiRagIndex 自身默认 INDEX_FILE 生效),
// 用户库分支落 knowledgeBaseIndexFile(spec.id)。
export async function buildKbIndex(kbId, { force = false, logger, indexPath = null } = {}) {
  const spec = await getKnowledgeBaseSpec(kbId);
  if (!spec) return { ok: false, error: `unknown knowledge base: ${kbId}` };
  if (spec.id === "wiki") return buildWikiRagIndex({ force, logger, ...(indexPath ? { indexPath } : {}) });
  ...
  return embedChunkPlanToIndex({ plan, indexPath: indexPath || knowledgeBaseIndexFile(spec.id), force, logger, temporal: spec.temporal === true, conflict: spec.conflict });
}
