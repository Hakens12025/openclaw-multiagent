...
export function resolveTrackingWorkItem(trackingState) {
  ...
  if (artifactContext) {
    ...
    return {
      ...
      ioObservation: trackingState?.ioObservation || artifactContext?.runtimeDiagnostics?.ioObservation || null,
...
export function resolveProgressWorkItem(entry) {
  ...
  return {
    ...
    ioObservation: normalizeTrackingObject(entry?.ioObservation)
      || normalizeTrackingObject(entry?.runtimeDiagnostics?.ioObservation),
    artifactKind,
