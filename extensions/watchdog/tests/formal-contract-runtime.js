...
const DEFAULT_CONTRACT_CASE_POLICY = Object.freeze({
  allowLoopElevation: false,
  requiredExecutionMode: "worker",
});

const CONTRACT_CASE_POLICIES = Object.freeze({
...
export function getFormalContractCasePolicy(testCaseOrId) {
  const caseId = typeof testCaseOrId === "string"
    ? testCaseOrId.trim()
    : String(testCaseOrId?.id || "").trim();
  return CONTRACT_CASE_POLICIES[caseId] || DEFAULT_CONTRACT_CASE_POLICY;
}
...
export function classifyContractExecutionMode({ contractRuntime } = {}) {
